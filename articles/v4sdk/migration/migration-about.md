...
- 새 Dialogs 라이브러리
  ...
  - 자세한 내용은 [대화 라이브러리](../bot-builder-concept-dialog.md)를 참조하세요.
- ASP.NET Core 지원
...
봇 어댑터를 만들 때 채널과 사용자로부터 들어오는 작업을 받을 턴 처리기 대리자도 제공합니다. 어댑터는 들어오는 각 작업에 대한 턴 컨텍스트 개체를 만듭니다. 턴 컨텍스트 개체를 턴 처리기에 전달한 다음, 턴이 완료되면 개체를 삭제합니다.

턴 처리기는 다양한 유형의 작업을 받을 수 있습니다. 일반적으로 _메시지_ 작업만 봇에 포함된 대화에 전달하려고 합니다. 작업 유형에 대한 자세한 내용은 [작업 스키마](https://aka.ms/botSpecs-activitySchema)를 참조하세요.

### <a name="handling-turns"></a>턴 처리

턴 처리기는 `BotCallbackHandler`에 대한 서명과 일치해야 합니다.
...
턴을 처리하는 경우 턴 컨텍스트를 사용하여 들어오는 작업에 대한 정보를 가져오고 사용자에게 작업을 보냅니다.

| | |
|-|-|
| 들어오는 작업 가져오기 | 턴 컨텍스트의 `Activity` 속성을 가져옵니다. |
| 작업을 만들어 사용자에게 보내기 | 턴 컨텍스트의 `SendActivityAsync` 메서드를 호출합니다.<br/>자세한 내용은 [문자 메시지 보내기 및 받기](../bot-builder-howto-send-messages.md) 및 [메시지에 미디어 추가](../bot-builder-howto-add-media-attachments.md)를 참조하세요. |
...
자세한 내용은 [상태 저장](../bot-builder-concept-state.md#saving-state)을 참조하세요.
...
- `WaterfallDialog` 클래스의 인스턴스인 폭포 대화입니다.

  이 옵션은 다양한 유형의 사용자 입력을 요청하고 유효성을 검사하는 프롬프트 대화에서 잘 작동하도록 설계되었습니다. [입력 프롬프트](../bot-builder-prompts.md)를 참조하세요.

  이렇게 하면 대부분의 프로세스가 자동화되지만 특정 양식이 대화 코드에 적용됩니다. [순차적 대화 흐름](../bot-builder-dialog-manage-conversation-flow.md)을 참조하세요. 그러나 대화 집합에 여러 대화를 추가하여 다른 제어 흐름을 만들 수 있습니다. [고급 대화 흐름](../bot-builder-dialog-manage-complex-conversation-flow.md)을 참조하세요.

- `ComponentDialog` 클래스에서 파생된 구성 요소 대화입니다.

  이 옵션을 사용하면 외부 컨텍스트와의 이름 없이 대화 코드를 캡슐화할 수 있습니다. [대화 재사용](../bot-builder-compositcontrol.md)을 참조하세요.

- `Dialog` 추상 클래스에서 파생된 사용자 지정 대화입니다.

  이 옵션은 대화의 작동 방식이 가장 유연하지만 대화 스택의 구현 방식에 대해서도 자세히 알고 있어야 합니다.

대화에 액세스하려면 해당 대화의 인스턴스를 _대화 집합_에 배치한 다음, 해당 집합에 대한 _대화 컨텍스트_를 생성해야 합니다.

대화 집합을 만들 때 대화 상태 속성 접근자를 제공해야 합니다. 이렇게 하면 프레임워크가 대화 상태를 한 턴에서 다음 턴으로 유지할 수 있습니다. [상태 관리](../bot-builder-concept-state.md)는 v4에서 상태를 관리하는 방법을 설명합니다.
...
- v4의 다양한 `Prompt` 파생 클래스는 사용자 프롬프트를 별도의 2단계 대화로 구현합니다. [대화 프롬프트를 사용하여 사용자 입력 수집](../bot-builder-prompts.md) 방법을 참조하세요.
