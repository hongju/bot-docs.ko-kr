...
v4에서 이 클래스는 봇이 순서를 시작하는 곳이며, v3 메시지 컨트롤러와는 전혀 다릅니다. 봇의 턴 처리기 자체를 제외한 대부분은 상용구로 생각할 수 있습니다.
...
점수 지정 가능 개체가 더 이상 존재하지 않으므로 `CancelScorable` 클래스를 복사하지 않습니다. 대화 상자에 제어를 전달하기 전에 순서 처리기에서 ‘global’ 명령을 확인할 수 있습니다. 
