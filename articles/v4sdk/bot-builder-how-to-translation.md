...
SDK에서 EchoBot 샘플을 시작하고, `Startup.cs` 파일에서 `ConfigureServices` 메서드를 업데이트하여 봇에 `TranslationMiddleware`를 추가합니다. 그러면 사용자로부터 받은 모든 메시지를 번역하도록 봇을 구성합니다. <!--, by simply adding it to your bot's middleware set. The middleware stores the translation results on the context object. -->
