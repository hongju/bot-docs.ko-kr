...
_활동 보내기_ 메서드와 해당 처리기에는 고유한 문제를 일으킵니다. _활동 보내기_ 처리기 내에서 단순히 _활동 보내기_ 를 호출하면 스레드가 무한히 포크됩니다. 이 문제는 나가는 정보에 추가 메시지를 추가하거나 봇이 충돌하지 않도록 다른 위치(예: 콘솔 또는 파일)에 기록하는 것과 같은 방법으로 해결할 수 있습니다.
