...
지식 봇은 일반적으로 대화를 안내하도록 설계될 때 가장 효과적입니다. 대화는 사용자와 봇 간의 교환으로 이루어지며, 이를 통해 기본 검색으로 할 수 없는 방식으로 확인 질문을 던지고 옵션을 제공하고 결과의 유효성을 검사하는 기회가 봇에 제공됩니다. 예를 들어 다음 봇은 사용자가 원하는 정보를 찾을 때까지 데이터 세트를 패싯 및 필터링하는 단계를 사용자에게 안내합니다.
...
기본 데이터 세트를 대표하는 예제 엔터티로 모델을 교육하면 봇의 언어 이해도를 높일 수 있습니다. 
...
- .NET용 Bot Framework SDK를 사용하여 기본 지식 봇을 만드는 방법을 보여주는 샘플은 GitHub의 <a href="https://aka.ms/qna-with-appinsights" target="_blank">기술 봇 샘플</a>을 참조하세요. 
